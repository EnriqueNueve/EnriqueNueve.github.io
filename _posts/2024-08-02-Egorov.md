...
**Lemma 1**: Suppose $$(X,\mathca{S},\mu )$$ is a measure space and $$E_1\subseteq E_2\subseteq \dots $$ is an increasing sequence of sets in $$\mathca{S}$$.
Then $$\mu (\cup_{k=1}^{\infty} E_k)=\lim_{k\to \infty }\mu (E_k)$$.

**Lemma 2**: Suppose $$(X,\mathca{S},\mu )$$ is a measure space and $D,E\in\mathca{S}$$ are such that $$D\subseteq E$$.
Then $\mu (E\setminus D)=\mu (E)-\mu (D)$$ provided thhat $$\mu (D)<\infty $$.
...
Note that each $$A_{m,n}\in\mathca{S}$$ since 
$$f_k-f$$ is 
$$\mathca{S}$$-measurable 
